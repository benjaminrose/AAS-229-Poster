...
Type Ia supernovae (\sn{}) are excellent distance indicators, but their Hubble residuals (HR) show some correlation with host galaxy properties. 
...
Research over the past several years is indicating that external variables seem to also contribute to \sn{} variability. There are still uncertainties about how the host galaxy and the local environment influence the luminosity, color, and HR of \sn{}. 
...
\end{equation}
Where $A_{\text{SF}}$ is the age of the universe at the given redshift minus $t_{\text{SF start}}$ and $\Psi(t)$ is the SFR as a function of time.
...
The HR were calecualted using Campbell's $\Lambda$CDM best fit: $\text{H}_{0}=73.8~\text{km s}^{-1}~\text{Mpc}^{-1}$, $\Omega_{\text{M}}=0.24$. The results of the HR versus local stellar population age is seen in \cref{fig:HRvAge}.
\end{block}
...
\begin{block}{So What?}
There is no visible trend between H$_0$ and local stellar age, but there is a bimodal distribution in the local age distribution.
\end{block}

\vskip1ex

\begin{block}{What is next}
Rigault finds a two mode model that at first glance does not fit my results. There also appears to be a difference between local age and global age, Gupta does not have a bimodal global age distribution. He has a mean of $\sim 4~\text{Gyr}$, in between my two peaks.
\end{block}
...
\includegraphics[width = 6.5in]{ND_mark_black.pdf}
\\ \vskip1ex
$~~~$\includegraphics[width = 6.5in]{astropy_powered.png}
