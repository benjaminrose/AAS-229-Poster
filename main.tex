...
The Hubble residuals were calculated using Campbell's $\Lambda$CDM best fit: $\text{H}_{0}=73.8~\text{km s}^{-1}~\text{Mpc}^{-1}$, $\Omega_{\text{m}}=0.24$. \Cref{fig:HRvAge} shows the results of the Hubble residual versus local stellar population age.
% The results of the Hubble residual versus local stellar population age is seen in \cref{fig:HRvAge}.
...
\begin{figure}
    ...
    % \includegraphics[height=9.5in]{HRvAge.pdf}
    \vskip-0.75ex
    \caption{A linear fit (blue line) was performed to look for a correlation between Hubble residual and local stellar age. Five binned points (blue) are plotted on top of the complete data set ($N=420$, green) to guide the eye. 
