...
Can using information about the local environment reduce Type Ia systematic
...
\begin{itemize}
    \item \sn{} selection, parameters, and cosmology by Campbell (spectroscopic and photometrically classified) \cite{Campbell13}
...
\begin{itemize}
    ...
    \item There appears to be a difference between local age and global age, Gupta does not have a bimodal global age distribution.%, but rather a mean of $\sim 4~\text{Gyr}$, in between my two peaks.
\end{itemize}
\end{block}
...
Investigate differences and similarities between global host parameters \cite{Gupta11} and local environment parameters, specifically age distribution. Also compare a mixed progenitor model to the model found in \cite{Rigault13}.
\end{block}
...
\begin{wrapfigure}{r}{0.45\onecolwid}
    ...
    \begin{center}
    \subfloat{
        \includegraphics[width=4in]{ND_mark_black.pdf}
    }
    
    \subfloat{
        \includegraphics[width=4in]{astropy_powered.png}
    }
    \end{center}
\end{wrapfigure}
%using small, but could also use footnotesize, but other sizes are too small.
